...
def load_mail() -> List[Dict[str, str]]:
    ...
    Returns: list: A list of dictionaries representing the mail entries
    """
    try:
...
def add_mail(mail_entry: Dict[str, str]) -> str:
    """
    Adds mail to mail_entry list and assigns it a unique ID 
    Returns: string ID for mail 
    """
    mail = load_mail()
...
def delete_mail(mail_id: str) -> bool:
    """
    Deletes item with given mail_id by command 
    Returns: bool true if email with id found and deletes it, else false 
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    """ Loads list of emails and checks to see if id of email matches the mail id provided
        Returns: entry: either the entry if found or none
    """
    mail = load_mail()
    for entry in mail:
        if entry['id'] == mail_id:
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """ loads list of mails and checks to see if the recipient in the parameter of function matches the recipient of email
        Returns: list: the list of emails to given recipient 
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    """
    Loads list of mails and checks to see if the sender matches the given sender 
    Returns: list: the list of emails from given sender
    """
    mail = load_mail()
    sent = []
    for entry in mail:
...
def add_mail_route():
    ...
    Returns: str: The id of the new mail entry
    """
...
def delete_mail_route(mail_id: str):
    ...
    if delete_mail(mail_id):
        response = jsonify({"message": f"Mail with ID {mail_id} deleted successfully."})
        response.status_code = 200
    else: 
        response = jsonify({"error": f"Mail with ID {mail_id} not found."})
...
def get_sender_route(sender: str):
    """
    Summary: Gets all mail entries sent by specific sender from the json file
