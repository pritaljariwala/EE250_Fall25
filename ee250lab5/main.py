...
def get_wifi_signal_strength() -> int:
    ...
    # dBm stands for decibel milliwatts, and it measures the strength of the signal relative to a mW. Since signal strength is measured 
    # in negative dBm, a value closer to 0 is typically considered stronger. For example, for WiFi, a measurement of greater than -70 dBm is considered "good".
    ...
    if platform.system() == 'Linux': # Linux
        ...
        output = subprocess.check_output("sudo wdutil info", shell=True)
        match = re.search(r"RSSI\s*:\s*(-?\d+)\s*dBm", output.decode('utf-8'))
        signal_strength = int(match.group(1))
    else:
...
def main():
    ...
    fig = px.bar(
        df, x = 'location', y = 'signal_strength_mean', error_y = 'signal_strength_std', title = "Signal strength vs Location"
    )
